...
class AWSProvider {
  constructor(targetConfig) {
    ...
    // Save credentials to AWS-recommended environment variables.
    process.env.AWS_ACCESS_KEY_ID = this.config_.aws_access_key_id;
    process.env.AWS_SECRET_ACCESS_KEY = this.config_.aws_secret_access_key;
  ...
  upload(localFileObj, cb) {
    ...
    async.auto({
      ...
      if (err) return cb(err);
      const sizeKb = Math.ceil(localFileObj.size / 1024);
      log.info(col.yellow(
        `Uploaded ${col.bold(localFileObj.relPath)}: ` +
        `Size: ${col.bold(sizeKb)} kb ` +
        `Cache-Control: ${col.bold(params.CacheControl)} ` +
        `Expires: ${col.bold(moment(params.Expires).fromNow())}`));
      return cb();
    });
  }
