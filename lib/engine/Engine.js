...
const _ = require('lodash'),
  ...
  path = require('path'),
  readBuildSpec = require('../util/spec').read,
...
class Engine extends EventEmitter {
  constructor(projectRoot, outputRoot, configSpec) {
    super();
    this.configSpec_ = configSpec;
    this.projectRoot_ = path.resolve(projectRoot);
    this.outputRoot_ = path.resolve(outputRoot);
    assert(fs.existsSync(this.projectRoot_));
    assert(fs.existsSync(this.outputRoot_));

    this.spec_ = readBuildSpec(projectRoot);
    log.debug(`Read build specification: ${json(this.spec_, null, 2)}`);
    ...
    this.wpConfigs_ = _.map(this.spec_.pages, (pageDef) => {
      ...
      const dummySrc = _.map(pageDef.styles, (styleRelPath) => {
        const absPath = path.join(projectRoot, styleRelPath);
        assert(
      ...
      _.forEach(pageDef.scripts, (scriptRelPath) => {
        entryPoints.push(path.join(projectRoot, scriptRelPath));
      });
      ...
      const dataObjects = _.map(pageDef.data, (dataRelPath) => {
        const absPath = path.join(projectRoot, dataRelPath);
        assert(
      ...
      return genWebpackConfig(
          pageDef, this.projectRoot_, this.outputRoot_, configSpec,
